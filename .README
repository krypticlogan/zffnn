...
This lib is not a PyTorch replacement, as it greatly restricts the flexibility in training due to having no runtime allocations or representation; it is however, an interesting look at what is possible with deterministic ML architecture.

While training is supported, the more natural use case is for inference on pretrained models in a constrained environment.
